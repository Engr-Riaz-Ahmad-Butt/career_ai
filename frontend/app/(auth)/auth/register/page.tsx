...
export default function RegisterPage() {
  ...
  const passwordStrength = Object.values(passwordStrengthChecks).filter(
    ...
    >
      <div className="rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 p-8 shadow-lg backdrop-blur-xl">
              ...
              >
                ...
                <div className="space-y-1 text-xs">
                  <div className="flex items-center gap-2">
                    {passwordStrengthChecks.length(password) ? (
                      <Check className="h-3 w-3 text-emerald-500" />
                    ) : (
                      <X className="h-3 w-3 text-slate-400" />
                    )}
                    <span className={passwordStrengthChecks.length(password) ? 'text-slate-600 dark:text-slate-400' : 'text-slate-500 dark:text-slate-500'}>
                      At least 8 characters
                    </span>
                  </div>
                  <div className="flex items-center gap-2">
                    {passwordStrengthChecks.uppercase(password) ? (
                      <Check className="h-3 w-3 text-emerald-500" />
                    ) : (
                      <X className="h-3 w-3 text-slate-400" />
                    )}
                    <span className={passwordStrengthChecks.uppercase(password) ? 'text-slate-600 dark:text-slate-400' : 'text-slate-500 dark:text-slate-500'}>
                      Uppercase letter
                    </span>
                  </div>
                  <div className="flex items-center gap-2">
                    {passwordStrengthChecks.lowercase(password) ? (
                      <Check className="h-3 w-3 text-emerald-500" />
                    ) : (
                      <X className="h-3 w-3 text-slate-400" />
                    )}
                    <span className={passwordStrengthChecks.lowercase(password) ? 'text-slate-600 dark:text-slate-400' : 'text-slate-500 dark:text-slate-500'}>
                      Lowercase letter
                    </span>
                  </div>
                  <div className="flex items-center gap-2">
                    {passwordStrengthChecks.number(password) ? (
                      <Check className="h-3 w-3 text-emerald-500" />
                    ) : (
                      <X className="h-3 w-3 text-slate-400" />
                    )}
                    <span className={passwordStrengthChecks.number(password) ? 'text-slate-600 dark:text-slate-400' : 'text-slate-500 dark:text-slate-500'}>
                      Number
                    </span>
                  </div>
                </div>
        ...
        <div className="my-6 flex items-center gap-4">
          ...
          <span className="text-xs text-slate-600 dark:text-slate-400">or continue with</span>
          <div className="flex-1 h-px bg-slate-200 dark:bg-slate-800" />
        </div>
